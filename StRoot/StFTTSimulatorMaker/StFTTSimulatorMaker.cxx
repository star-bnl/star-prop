...
StFTTSimulatorMaker::StFTTSimulatorMaker(const Char_t *name)
    : StMaker(name),
      hGlobalYX(0),
      hOctantYX(0),
      hOctantWireYX(0),
...
Int_t StFTTSimulatorMaker::Make() {
    LOG_INFO << "StFTTSimulatorMaker::Make" << endm;
 ...
 */
void StFTTSimulatorMaker::sTGCGlobalToLocal(float x, float y, int disk, int &quad, float &localX, float &localY) {
    // quad RECT
    float qr = -1;
    float ql = -1;
    float qb = -1;
...
float StFTTSimulatorMaker::diskOffset(int disk) {
...
float StFTTSimulatorMaker::diskRotation(int disk) {
    assert(disk >= 9 && disk <= 12);
    // these are
    if (disk == 9)
...
void StFTTSimulatorMaker::sTGCQuadBottomLeft(int disk, int quad, float &bottom, float &left) {
 ...
 */
void StFTTSimulatorMaker::sTGCLocalToGlobal(float localX, float localY, int disk, int quad, float &globalX, float &globalY) {
    // quad RECT
    float qb = -1;
    float ql = -1;
 ...
 * 
 */
bool StFTTSimulatorMaker::sTGCOverlaps(StRnDHit *hitA, StRnDHit *hitB) {
...
void StFTTSimulatorMaker::fillThinGapChambers(StEvent *event) {
    // Read the g2t table
